  ...
  const handleLogin = async () => {
    ...
    try {
      if (isEmail(trimmed)) {
        ...
        const me = await apiRequest<{ role: string; email?: string | null }>('/auth/me', {
        ...
        loginAsAdmin(
          String(me.email || trimmed),
          normalizedRole as 'gestionnaire' | 'super_admin',
          tokenResponse.access_token,
          tokenResponse.must_change_password,
        );
        return;
      }
      ...
      const tokenResponse = await apiRequest<{ access_token: string; must_change_password: boolean }>('/auth/login-parent', {
        method: 'POST',
        body: JSON.stringify({ matricule: trimmed, password }),
      });
      ...
      const parentProfile = me.parent
        ? {
            ...
            site: me.parent.site_code || undefined,
          }
          ...
          >
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="identifier" placeholder="Saisissez votre matricule ou votre adresse e-mail" value={identifier} onChange={e => setIdentifier(e.target.value)} className="pl-10 h-11 rounded-lg" />
              </div>
            </div>
