...
import { LayoutDashboard, UserPlus, Users, HelpCircle, Eye, Award } from 'lucide-react';
...
const allItems = [
  ...
  { title: 'Liste finale', id: 'liste_finale_parent', icon: Award },
];
...
export function ParentSidebar({ currentPage, onNavigate }: Props) {
  ...
  const { settings, getEnfantsByParent, listeFinaleGeneree } = useInscription();
  ...
  // Filter items based on inscription status
  let items = inscriptionsCloturees
    ? allDesistes
      ? allItems.filter(i => i.id === 'dashboard')
      : allItems.filter(i => i.id !== 'inscrire')
    : allItems.filter(i => i.id !== 'liste_finale_parent');

  // Only show Liste finale if it has been generated
  if (!listeFinaleGeneree) {
    items = items.filter(i => i.id !== 'liste_finale_parent');
  }
