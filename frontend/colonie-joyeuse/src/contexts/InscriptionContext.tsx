...
export function InscriptionProvider({ children }: { children: ReactNode }) {
  ...
  const isInscriptionsCloturees = useMemo(() => {
    if (!settings.dateFinInscriptions) return false;
    const now = new Date();
    const dateFin = new Date(`${settings.dateFinInscriptions}T23:59:59`);
    return now > dateFin;
  }, [settings.dateFinInscriptions]);

  useEffect(() => {
    if (isInscriptionsCloturees) setListeFinaleGeneree(true);
  }, [isInscriptionsCloturees]);
  ...
  const reloadParentDemandes = async () => {
    ...
    const mapped: Enfant[] = demandes.map((d) => {
      ...
      return {
        ...
        dateDesistement: d.date_desistement || undefined,
      };
    });
    setEnfants(mapped);
    setParents([{
      ...
      site: authParent.site || authParent.site_code || undefined,
      motDePasse: '',
      email: authParent.email,
      telephone: authParent.telephone,
    }]);
  };
  ...
  const reloadAdminDemandes = async () => {
        ...
        users
          ...
          .forEach((u) => {
            ...
            parentByMatricule.set(mat, {
              ...
              site: String(u.parent_site_code || cur?.site || ''),
              telephone: String(u.parent_telephone || cur?.telephone || ''),
              email: String(u.email || cur?.email || ''),
              motDePasse: '',
            });
          });
      } catch {
    ...
    const desistementMap = desistements.reduce((acc: Record<number, number>, d) => {
      if (typeof d.demande_id === 'number' && typeof d.desistement_id === 'number') acc[d.demande_id] = d.desistement_id;
      return acc;
    }, {});
  ...
  const reloadHistorique = async () => {
    if (!token || (role !== 'gestionnaire' && role !== 'super_admin')) return;
    try {
      const rows = await apiRequest<any[]>('/admin/historique?limit=200', { token });
      setHistorique(rows.map((r, idx) => ({
        id: String(r.id || `h_${idx}`),
        date: String(r.date || ''),
        heure: String(r.heure || ''),
        utilisateur: String(r.utilisateur || ''),
        role: String(r.role || ''),
        action: String(r.action || ''),
        details: String(r.details || ''),
        cible: r.cible ? String(r.cible) : undefined,
      })));
    } catch {
      setHistorique([]);
    }
  ...
  const updateSettings = (s: Partial<AppSettings>) => {
    ...
    if (token && role === 'super_admin') {
      apiRequest('/admin/settings', {
        ...
        // ignore write failure; local state stays optimistic
      });
    }
  };
  ...
  const validerDesistement = async (enfantId: string) => {
    if (token && (role === 'gestionnaire' || role === 'super_admin')) {
      ...
      if (!desistementId) throw new Error('Désistement introuvable pour cette demande.');
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // ignore transient API errors in UI
      }
    };
    loadData();
