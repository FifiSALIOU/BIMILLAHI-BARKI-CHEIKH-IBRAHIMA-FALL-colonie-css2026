...
export default function GestionListesConfig() {
  ...
  const loadListes = async () => {
    ...
    setLoading(true);
    try {
      const data = await apiRequest<Array<{ id: number; code: string; nom: string; description?: string | null }>>('/admin/listes-config', { token });
      setListes(
        data.map((l) => ({
          id: String(l.id),
  ...
  const handleImportListes = async (data: any[]): Promise<ImportResult> => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (listes.some(l => l.code === code)) { errors.push({ ligne: i + 2, message: `Code "${code}" déjà existant` }); continue; }
      try {
        await apiRequest('/admin/listes-config', {
          method: 'POST',
          token,
          body: JSON.stringify({ code, nom: row.nom, description: row.description || '' }),
        });
        success++;
      } catch (e) {
