...
export default function GestionSites() {
  ...
  const loadSites = async () => {
    ...
    setLoading(true);
    try {
      const data = await apiRequest<Array<{ id: number; nom: string; code: string | number; description?: string | null }>>('/admin/sites', { token });
      setSites(
        data.map((s) => ({
          id: String(s.id),
          nom: s.nom,
  ...
  const handleImportSites = async (data: any[]): Promise<ImportResult> => {
    let success = 0;
